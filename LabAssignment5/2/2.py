...
def render(M, camAng): 
    ...
    gluLookAt(0.1*np.sin(camAng),0.1,0.1*np.cos(camAng), 0,0,0, 0,1,0)
...
def key_callback(window, key, scancode, action, mods): 
    ...
    if action==glfw.PRESS or action==glfw.REPEAT:
        if key==glfw.KEY_1: 
            ...
            temp[0,-1] = -0.1
            gComposedM = temp @ gComposedM
        elif key == glfw.KEY_E:
            temp[0,-1] = 0.1
            gComposedM = temp @ gComposedM
        elif key == glfw.KEY_A:
            temp[:-1,:-1] = np.array([[np.cos(np.radians(-10)),0,np.sin(np.radians(-10))],
                               [0,1,0],
                               [-np.sin(np.radians(-10)),0,np.cos(np.radians(-10))]])
            gComposedM = gComposedM @ temp
        elif key == glfw.KEY_D:
            temp[:-1,:-1] = np.array([[np.cos(np.radians(10)),0,np.sin(np.radians(10))],
                               [0,1,0],
                               [-np.sin(np.radians(10)),0,np.cos(np.radians(10))]])
